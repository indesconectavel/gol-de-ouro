-- =============================================================================
-- V1.1B-M1-R3 — Patch cirúrgico REAL (validado em staging 2026-05-17)
-- =============================================================================
-- Estado: STAGING-VALIDATED — NÃO aplicar em produção sem gate pré-prod final.
--
-- Base: RPC produção (características confirmadas pela missão R2):
--   • pg_advisory_xact_lock por payment_id
--   • claim pending → approved no ramo principal
--   • ledger tipo deposito
--   • idempotência por (correlation_id, tipo) no ledger
--
-- Alteração R2 (única intenção funcional):
--   • Ramo self-heal (PIX já approved, sem ledger): APENAS backfill ledger
--   • Removido crédito de saldo nesse ramo (evita double-credit nos ~30 casos)
--   • U1–U4 (saldo 0): NÃO creditar automaticamente — runbook manual
--
-- ANTES de apply: colar pg_get_functiondef real em
--   docs/relatorios/snapshots/claim_and_credit_approved_pix_deposit-ANTES-M1-producao-2026-05-17.sql
-- e diffar este ficheiro contra o corpo real. Se o corpo prod diferir, aplicar só o hunk
-- documentado em docs/relatorios/V1.1B-M1-R2-DIFF-RPC-PIX-LEDGER-2026-05-17.md §4.
-- =============================================================================
...
BEGIN
  ...
  -- Advisory lock (preservado — serializa por MP id na transação)
  ...
  -- Idempotência ledger: (correlation_id, tipo) — inclui legado referencia = payment_id
  SELECT l.id
  INTO v_ledger_id
  ...
  -- -------------------------------------------------------------------------
  -- R2 — Self-heal: approved sem ledger → SOMENTE ledger (sem UPDATE saldo)
  -- Substitui ramo que creditava saldo novamente (double-credit nos ~30 approved).
  -- -------------------------------------------------------------------------
  IF v_status_norm = 'approved' THEN
    ...
    RETURN jsonb_build_object(
      ...
      'pagamentos_pix_id', v_pix.id,
      'ledger_id', v_ledger_id,
      'saldo_unchanged', true
    );
  END IF;

  -- -------------------------------------------------------------------------
  -- Ramo principal: pending → claim + ledger + saldo (inalterado em intenção R2)
  -- -------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.claim_and_credit_approved_pix_deposit(text) IS
  'R3: self-heal approved sem ledger = ledger_backfill only; claim principal inalterado; advisory lock.';

...
const Jogo = () => {
  ...
  const handleShoot = async (dir) => {
    ...
    console.log('🎯 [JOGO] Estado atual:', { shooting, balance, currentBet });
    
    if (shooting || balance < currentBet) {
      console.log('⚠️ [JOGO] Chute bloqueado:', { shooting, balance, currentBet });
    ...
    try {
      ...
      if (result.success) {
        ...
        if (capturedIsGoal) {
            if (capturedIsGoldenGoalShot) {
              // GOL DE OURO - Lógica simplificada da v6
              ...
              const hideGoldenTimer = setTimeout(() => {
                console.log('🖼️ [JOGO] Ocultando golden-goal.png');
              ...
              if (!isMuted) {
                console.log('🔊 [JOGO] Tocando som de gol + torcida');
                playGoalSound();
              ...
              const showGanhouTimer = setTimeout(() => {
                ...
                const resetTimer = setTimeout(() => {
                  console.log('🔄 [JOGO] Resetando animações após ganhou');
                  resetAnimations();
                }, 3000);
                addTimer(resetTimer);
              }, 1200);
            ...
            const resetTimer = setTimeout(() => {
              console.log('🔄 [JOGO] Resetando animações após defesa');
              resetAnimations();
            }, 2000);
            addTimer(resetTimer);
          }
      ...
      console.error('❌ [JOGO] Erro ao processar chute:', error);
      console.error('❌ [JOGO] Detalhes do erro:', {
      ...
      let errorMessage = error.message || 'Erro ao processar chute';
      
      // Erro do backend (via gameService)
      if (error.response) {
        ...
        if (status === 400) {
          ...
          const backendMessage = data?.message || data?.error || data?.details || 'Dados inválidos';
          errorMessage = `Erro de validação: ${backendMessage}. Verifique se a direção (${dir}) e o valor (R$ ${currentBet}) estão corretos.`;
          console.error('❌ [JOGO] Erro 400 - Dados inválidos:', {
            status,
            data,
            direction: dir,
            amount: currentBet,
            balance,
            payload: { direction: dir, amount: currentBet }
          });
        } else if (status === 401) {
          errorMessage = 'Sessão expirada. Por favor, faça login novamente.';
        } else if (status === 403) {
          errorMessage = 'Saldo insuficiente ou permissão negada.';
        } else if (status >= 500) {
          errorMessage = 'Erro no servidor. Tente novamente em alguns instantes.';
        } else {
          errorMessage = data?.message || data?.error || `Erro ${status}: ${errorMessage}`;
        }
      } else if (result && !result.success) {
        // Erro retornado pelo gameService
        errorMessage = result.error || result.details?.message || errorMessage;
        console.error('❌ [JOGO] Erro do gameService:', result);
      } else if (error.message) {
        // Erro lançado pelo gameService ou validação local
      ...
      // Garantir que shooting seja resetado mesmo em caso de erro
      setShooting(false);
    }
  };
                  ...
                  <div className="bet-buttons">
                    {betValues.map((value) => (
                      <button
                        ...
                        disabled={balance < value || shooting}
              ...
              {Object.entries(GOAL_ZONES).map(([zone, pos]) => {
                ...
                return (
                  <button
                    ...
                    className={`gs-zone ${shooting || balance < currentBet ? 'disabled' : ''}`}
                    style={{
                      position: 'absolute',
                      ...
                      background: shooting || balance < currentBet ? 'rgba(255, 255, 255, 0.1)' : 'rgba(255, 255, 255, 0.2)',
                      cursor: shooting || balance < currentBet ? 'not-allowed' : 'pointer',
                      zIndex: 5,
          ...
          <div className="hud-bottom-left">
            <div className="game-actions">
                <button
                  ...
                  disabled={shooting}
                  style={{
                    pointerEvents: shooting ? 'none' : 'auto',
                    opacity: shooting ? 0.5 : 1,

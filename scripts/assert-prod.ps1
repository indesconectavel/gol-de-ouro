# scripts/assert-prod.ps1 - Validação estrita de produção (sem falso-positivo)
...
function Write-Fail {
    ...
    Write-Host "💡 Dica: Verifique a configuração e tente novamente" -ForegroundColor Yellow
    exit 1
}

Write-Host "🔎 VALIDAÇÃO ESTRITA DE PRODUÇÃO - GOL DE OURO" -ForegroundColor Cyan
...
try {
    ...
    if ($versionResponse.StatusCode -eq 200) {
        ...
        if ($versionJson.version -and $versionJson.commit) {
            ...
            Write-Fail "Version endpoint não contém version/commit: $($versionResponse.Content)"
        }
    } else {
        Write-Assert "GET /version → 200 com version e commit" "FAIL"
...
# 3) GET $ApiBase/readiness → 200 e validação de DB
...
try {
    ...
    if ($readinessResponse.StatusCode -eq 200) {
        ...
        if ($readinessJson.status -eq "ready" -or $readinessJson.database) {
            Write-Assert "GET /readiness → 200 e validação de DB" "OK"
            Write-Host "   Resposta salva: $($readinessResponse.Content)" -ForegroundColor Gray
        } else {
            Write-Assert "GET /readiness → 200 e validação de DB" "FAIL"
            Write-Fail "Readiness endpoint não validou DB: $($readinessResponse.Content)"
        }
    } else {
        Write-Assert "GET /readiness → 200 e validação de DB" "FAIL"
...
try {
    ...
    if ($playerResponse.StatusCode -eq 200) {
        ...
        Write-Host "   Cabeçalhos salvos: $($playerResponse.Headers.Count) headers" -ForegroundColor Gray
...
try {
    ...
    if ($corsResponse.StatusCode -eq 200 -or $corsResponse.StatusCode -eq 204) {
        ...
        if ($hasOrigin -and $hasMethods -and $hasHeaders) {
            ...
            Write-Host "   Cabeçalhos CORS salvos:" -ForegroundColor Gray
...
# 7) PWA em produção: manifest e sw.js
Write-Host "`n7. Verificando PWA em produção..." -ForegroundColor Yellow
...
if ($global:ASSERT_FAILED) {
    Write-Fail "Validação estrita falhou - sistema NÃO está pronto para produção"
} else {
    Write-Host "✅ TODOS OS ASSERTS PASSARAM - Sistema pronto para produção!" -ForegroundColor Green
}

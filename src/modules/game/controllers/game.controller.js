// Game Controller - Gol de Ouro v1.3.0 - PADRONIZADO V19
// Módulo: game
// Data: 2025-01-24
...
const LoteIntegrityValidator = require('../../shared/validators/lote-integrity-validator');
...
class GameController {
  ...
  async shoot(req, res) {
    try {
      ...
      // ✅ CORREÇÃO: Normalizar tipos antes de validar
      ...
      if (!normalizedDirection || !normalizedAmount || isNaN(normalizedAmount)) {
        console.error('❌ [SHOOT] Dados inválidos recebidos:', { 
      ...
      // ✅ CORREÇÃO: Validar direção explicitamente
      ...
      // Usar valores normalizados daqui em diante
      ...
      if (user.saldo < finalAmount) {
        console.error('❌ [SHOOT] Saldo insuficiente:', { 
          saldo: user.saldo, 
          amount: finalAmount,
        ...
        return res.status(400).json({
          success: false,
          message: `Saldo insuficiente. Saldo atual: R$ ${user.saldo.toFixed(2)}, necessário: R$ ${finalAmount.toFixed(2)}`
        });
      }

      // ✅ CORREÇÃO CRÍTICA: Debitar saldo ANTES de processar o chute (ACID)
      console.log(`💰 [SHOOT] Debitando R$ ${finalAmount} do usuário ${req.user.userId}...`);
      const deductResult = await FinancialService.deductBalance(
        req.user.userId,
        finalAmount,
        {
          description: `Aposta no jogo - Chute ${finalDirection}`,
      ...
      const lote = await getOrCreateLoteByValue(finalAmount);
      ...
      if (!integrityValidation.valid) {
        ...
        return res.status(400).json({
          success: false,
          message: integrityValidation.error || 'Erro de validação do lote',
      ...
      lote.totalArrecadado += finalAmount;

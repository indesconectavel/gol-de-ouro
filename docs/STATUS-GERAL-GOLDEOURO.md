...
**Última atualização:** 2026-03-26 *(BLOCO F — encerramento documental do refinamento app shell)*  
...
O **refinamento do app shell** (BLOCO F — botões, layout interno, safe-area no footer) está **documentalmente encerrado e validado** em 2026-03-26 (`docs/relatorios/BLOCO-F-ENCERRAMENTO-OFICIAL-2026-03-26.md`), **sem alteração** a ficheiros de gameplay. Permanece em evolução a **Fase I.5 — Idempotência do chute** (validação em ambiente de preview). Questões históricas de **preview vs produção** em overlays da `/game` podem continuar relevantes para **governança de deploy** e devem ser cruzadas com a branch/commit em uso. A **produção permanece protegida** (referência de rollback: tag `pre-fase1-idempotencia-2026-03-17`). O veredito geral do ciclo de **promoção preview → produção** pode continuar **BLOQUEADO** enquanto I.5 e critérios de deploy não forem satisfeitos — **independentemente** do encerramento do BLOCO F para UI shell.
...
- **Status:** 🟢 **FECHADO / VALIDADO** *(refinamento app shell, 2026-03-26)*
- **Escopo:** telas do player web, fluxo de navegação, UX, feedback visual; neste ciclo: **padronização de botões** (SUPER PRIMARY, PRIMARY, SECONDARY, TERTIARY), `InternalPageLayout`, fluxos Login/Register/Pagamentos/Withdraw/Profile, safe-area no footer, ajuste disabled no Withdraw.
- **Observações:** O encerramento documental refere-se ao **app shell** e **não** altera `GameFinal.jsx`, `game-scene.css` nem `game-shoot.css` — **gameplay intocado**. Relatório mestre: `docs/relatorios/BLOCO-F-ENCERRAMENTO-OFICIAL-2026-03-26.md`. Para **histórico** de preview/overlays da `/game`, manter referência a relatórios forenses e a `RELATORIO-FINAL-BLOCOS-ATE-F-2026-03-18.md`; decisões de **deploy** seguem sujeitas a I.5 e a políticas de branch.
...
2. **BLOCO F — Interface (app shell):** refinamento de botões e layout interno **encerrado documentalmente** em 2026-03-26. **Overlays / preview vs produção** na `/game`: se ainda aplicável ao branch atual, seguir relatórios forenses e validação de deploy (fora do escopo do encerramento app shell).
